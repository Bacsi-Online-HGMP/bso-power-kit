#!/usr/bin/env bash
# The same book, but a cross-reference at the start of a line was cut as a third
# "chapter", and 00-contents.md carries the split warning the script writes for it.
set -euo pipefail
...
FIXTURE
cat > digests/text/sleep-science/03-chapter-1-showed-that-a-nap-helps-recall.md <<'FIXTURE'
---
type: Source Material
title: "sleep-science - part 03"
description: "Part 03 of sleep-science.txt: Chapter 1 showed that a nap helps recall; the same logic of timing applies to light."
generated: { by: process:extract_book, at: "2026-09-25T16:31:44+00:00" }
---

# Chapter 1 showed that a nap helps recall; the same logic of timing applies to light.

(Source: sleep-science.txt)

Morning light pulls the clock earlier, so a bright morning partly cancels a late evening.
FIXTURE
...
3 parts, 757 words. Split by chapter headings in the text.
...
- `03-chapter-1-showed-that-a-nap-helps-recall.md` - Chapter 1 showed that a nap helps recall; the same logic of timing applies to light. (24 words)
...
- part 03 repeats number 1 (already used by part 01) -- 'Chapter 1 showed that a nap helps recall; the same logic of timing applies to light.'
FIXTURE
