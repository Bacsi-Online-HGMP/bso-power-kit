...
def main(argv: list[str] | None = None) -> int:
    ...
    scan_source_tree()
    artifacts = [
        build_zip(dist / f"anti-slop-brain-template-v{version}.zip", REPO / "assets" / "template-brain", "anti-slop-brain-template"),
        build_zip(dist / f"anti-slop-brain-sample-vault-v{version}.zip", REPO / "examples" / "sample-vault", "anti-slop-brain-sample-vault"),
        build_source_zip(dist / f"anti-slop-brain-source-v{version}.zip", version),
    ]
    for artifact in artifacts:
        validate_zip(artifact["path"])
    manifest = {
        "product": "anti-slop-brain",
        "version": version,
...
def build_source_zip(out: Path, version: str) -> dict[str, object]:
    ...
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for path in source_files():
            rel = path.relative_to(REPO)
            if not should_skip(rel):
                zf.write(path, (Path(f"anti-slop-brain-v{version}") / rel).as_posix())
                entries += 1
    return {"path": out, "entries": entries}
