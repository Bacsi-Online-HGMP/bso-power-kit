...
Installs the Anti-Slop Brain skill surface.
Use --target custom --path <agent-skill-root> for Hermes or another runtime
when its official skill root is known.
Set ANTI_SLOP_BRAIN_INSTALL_HOME to test against a temporary home directory.
USAGE
}
...
install_one() {
  ...
  if [ -d "${source_dir}/agents" ]; then
    copy_tree "${source_dir}/agents" "${dest}/agents"
  fi
...
PY
  echo "Anti-Slop Brain Gemini loader updated at ${loader}"
}
